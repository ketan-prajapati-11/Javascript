...
/* ===================== userQuizData: persistent history of every finished quiz =====================
   Unlike `quizResult` (which is just a single-slot handoff to result.html/
   review.html and gets overwritten by the next quiz), `userQuizData` is
   never overwritten — every completed quiz is appended to it, so it builds
   up into a full history across sessions. Safe to read this back later
   for a "past attempts" / stats page. */
function saveToUserQuizData(result) {
  // Existing history, or a fresh array the very first time this runs.
  const history = JSON.parse(localStorage.getItem('userQuizData') || '[]');

  const scorePercentage =
    result.totalQuestions > 0 ? Math.round((result.correctCount / result.totalQuestions) * 100) : 0;

  history.push({
    // id: Date.now(), // good enough as a unique id for a single-user local history
    ...
    scorePercentage,
    // answers: result.answers, // full per-question breakdown, same shape review.js already uses
    completedAt: new Date().toISOString()
  });

  localStorage.setItem('userQuizData', JSON.stringify(history));
}
...
function finishQuiz() {
  ...
  saveToUserQuizData(quizResult); // append this attempt to the permanent history
